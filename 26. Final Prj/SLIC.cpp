...
#ifdef FINAL
extern int tmpx[1030] = {0, };
extern int tmpy[1030] = {0, };
extern super sup[1500] = {0, };
#endif
//////////////////////////////////////////////////////////////////////
	...
	const vector<double>& edgemag)
{
	...
	for (int y = 0; y < ystrips; y++)
	{
		...
		for (int x = 0; x < xstrips; x++)
		{
			...
			sup[n].cx = seedx;
			sup[n].cy = seedy;
			//printf("sup[%d] l = %.1f, a = %.1f, b = %.1f, x = %d, y = %d\n", n, sup[n].l, sup[n].a, sup[n].b, sup[n].cx, sup[n].cy);
#endif
			n++;
		}
	}
...
//----------------
...
#ifdef FINAL
	//cout << endl << "**PerformSuperpixelSLIC**" << endl;
	...
	for (int itr = 0; itr < 10; itr++)
	{
		...
		{for (int k = 0; k < numk; k++) //numK : num of Superpixel
		{
			kseedsl[k] = sigmal[k] * inv[k];
			kseedsa[k] = sigmaa[k] * inv[k];
			kseedsb[k] = sigmab[k] * inv[k];
			kseedsx[k] = sigmax[k] * inv[k];
			//cout << "sigmax[" << k << "] = " << sigmax[k] << endl;
			...
			sup[k].cx = kseedsx[k];
			sup[k].cy = kseedsy[k];
			//printf("sup[%d] l = %.1f, a = %.1f, b = %.1f, x = %d, y = %d\n", k, sup[k].l, sup[k].a, sup[k].b, sup[k].cx, sup[k].cy);
#endif
		}}
	}
#ifdef FINAL
	//printf("\nfinal superpixel eliment\n");
	//for (int k = 0; k < numk; k++) {
	//	printf("sup[%2d] l = %.1f, a = %.1f, b = %.1f, x = %3d, y = %3d\n", k, sup[k].l, sup[k].a, sup[k].b, sup[k].cx, sup[k].cy);
	//}
	//printf("\n");
#endif
}
	...
	const int& K) //the number of superpixels desired by the user
{
	...
	//	const int dy8[8] = { 0, -1, -1, -1, 0, 1, 1,  1};
#ifdef FINAL
	//cout << endl << "**EnforceLabelConnectivity**" << endl;
#endif
...
//#ifdef FINAL
//							printf("n = %d, nindex = %d, adjlabel = %d\n", n, nindex, adjlabel);
//#endif
						}
					}
				}}
...
{
#ifdef FINAL
	//cout << endl << "**DoSuperpixelSegmentation_ForGivenSuperpixelSize**" << endl;
#endif
	//------------------------------------------------
...
#ifdef FINAL
	//printf("x size : %u, y size : %u\n", kseedsx.size(), kseedsy.size());

	for (int i = 0; i < kseedsx.size(); i++) {
		//printf("%dth x : %.0f , y: %.0f\n", i, kseedsx[i], kseedsy[i]);
		tmpx[i] = kseedsx[i];
		tmpy[i] = kseedsy[i];
		//pix[i].cx = kseedsx[i];
		//pix[i].cy = kseedsy[i];
...
//merge pixel func
#ifdef FINAL
void Merge(const unsigned int* ubuff,int*& labels, int width, int height, int& numlabels, const int& K, const double& compactness) {
	printf("\nbefore numlabels = %d\n", numlabels);
	//to get TH
	int x, y;
	double sum = 0;
	...
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++) {
			int l = ubuff[y * width + x] >> 16 & 0xff;
			int a = ubuff[y * width + x] >> 8 & 0xff;
			int b = ubuff[y * width + x] & 0xff;
			sum += sqrt(l * l + a * a + b * b);
		}
	}
	
	double multi = 0.3; //when superpixel num = 1000, if superpixel num increase, multi value increase too.
	
	TH = multi * sum / (width * height);
	//printf("\n************TH : %f***********\n", TH);

	int maxpix = -1;
	maxpix = 0.1 * K * width * height / numlabels; //maxpixel value : superpixel num * 0.1
	//printf("maxpix : %d\n", maxpix/20);
	double dist = 0;
	double new_compactness = 0.05 * compactness; //compactness multi value set depend on superpixel num(0.05~0.1)
	int mergecnt = 0;
	for (int i = 0; i < numlabels - 1; i++) {
		double D = 10000;
		double Dc = 10000;
		double Ds = 10000;
		if (sup[i].cx > sup[i + 1].cx) i++;
		dist = sqrt((sup[i + 1].cx - sup[i].cx) * (sup[i + 1].cx - sup[i].cx) + (sup[i + 1].cy - sup[i].cy) * (sup[i + 1].cy - sup[i].cy));
		dist = dist*1.2;
		for (x = sup[i].cx - dist; x <= sup[i].cx + dist; x++) {
			for (y = sup[i].cy - dist; y <= sup[i].cy + dist; y++) {
				if (x >= 0 && x <= width && y >= 0 && y <= height) {
					for (int k = 0; k < numlabels; k++) {
						if (i != k && x == sup[k].cx && y == sup[k].cy) {
							//printf("%d and %d adjacent\n", i, k);
							Dc = sqrt((sup[k].l - sup[i].l) * (sup[k].l - sup[i].l) + (sup[k].a - sup[i].a) * (sup[k].a - sup[i].a) + (sup[k].b - sup[i].b) * (sup[k].b - sup[i].b));
							Ds = sqrt((sup[k].cx - sup[i].cx) * (sup[k].cx - sup[i].cx) + (sup[k].cy - sup[i].cy) * (sup[k].cy - sup[i].cy));
							D = sqrt(Dc * Dc + new_compactness * Ds * Ds);
							//printf("D : %f\n",D);
							if (D < TH) {
								//printf("Merge %d and %d labels\n", i, k);								
//#define TEST
#ifdef TEST
								if (sup[i].npix + sup[k].npix < maxpix) {
									mergecnt++;
									//printf("Execute with %d and %d sup\n", i, k);
									for (int yy = 0; yy < height; yy++) {
										for (int xx = 0; xx < width; xx++) {
											if (labels[yy * width + xx] == k) labels[yy * width + xx] = i;
											if (labels[yy * width + xx] > k) labels[yy * width + xx]--;
										}
									}
									sup[i].cx = (sup[i].cx + sup[k].cx) / 2;
									sup[i].cy = (sup[i].cy + sup[k].cy) / 2;
									sup[i].npix = sup[i].npix + sup[k].npix;
									for (int n = k + 1; n < numlabels; n++) {
										sup[n - 1] = sup[n];
									}
									//numlabels--;
								}
	...
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++) {
			if (new_labelnum < labels[y * width + x]) new_labelnum = labels[y * width + x];
		}
	}
	...
	for (y = 0; y < height; y++) { 
		for (x = 0; x < width; x++) {
			if (labels[y * width + x] == 1) labels[y * width + x] = 0;
			if (labels[y * width + x] > 1) labels[y * width + x]--;
		}
	}
	//if two suppix merged, how to resign cx,cy,color? and how to resign sup[] arr?
	sup[0].cx = (sup[0].cx + sup[1].cx) / 2;
	sup[1].cy = (sup[0].cy + sup[1].cy) / 2;
	sup[0].npix = sup[0].npix + sup[1].npix;
	for (int n = 2; n < numlabels; n++) {
		sup[n - 1] = sup[n];
	}
	numlabels--;
#endif
...
#ifdef FINAL
	//cout << endl << "**DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels**" << endl;
...
#ifdef FINAL
	for (int k = 0; k < numlabels; k++) sup[k].npix = 0;
	for (int k = 0; k < numlabels; k++) {
		for (int j = 0; j < width * height; j++) {
			if (klabels[j] == k) sup[k].npix++;
		}
	}
	//for (int k = 0; k < numlabels; k++) printf("# pixel of sup[%d] = %d\n", k, sup[k].npix);
	Merge(ubuff, klabels, width, height, numlabels, K, compactness);
#endif
}
